...
import { AnimatedLink, AnimatedRouteLink } from '../components/animated-link'
    ...
    <main>
      ...
      <section className="page-frame py-20 md:py-28" id="featured">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {homeFeaturedItems.map((item) => {
            ...
            return item.variant === 'note'
              ? <Link className={className} key={item.variant} params={{ slug: item.slug }} resetScroll={true} to="/notes/$slug">{content}</Link>
              : <Link className={className} key={item.variant} params={{ slug: item.slug }} resetScroll={true} to="/work/$slug">{content}</Link>
          })}
        </div>
      </section>
      ...
      <section className="border-t border-ink/20 bg-plum px-5 py-16 text-cream md:px-8 md:py-24">
        <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-12">
          ...
          <AnimatedRouteLink className="round-link-inverse h-fit border-cream/70 md:col-span-2" to="/about">About me ↗</AnimatedRouteLink>
        </div>
      </section>
    </main>
  )
